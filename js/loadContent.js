...
function renderArticle({ title, content }) {
  ...
  articleDiv.innerHTML = `
    ...
    <p>${escapeHtml(content.substring(0, 150))}${content.length > 150 ? '...' : ''}</p>
...
function renderInfographic({ imageUrl, caption }) {
  ...
  infographicDiv.innerHTML = `
    <img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(caption)}" loading="lazy" onerror="handleImageError(this)">
...
function showEmptyState(container) {
  const sectionName = container.id.replace('-list', '').replace(/([A-Z])/g, ' $1').toLowerCase();
  const icons = {
...
setInterval(() => {
  document.querySelectorAll('iframe[data-src]:not([data-lazy-observed])').forEach(iframe => {
